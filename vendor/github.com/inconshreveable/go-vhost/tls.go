...
func readClientHello(rd io.Reader) (*ClientHelloMsg, error) {
	...
	readRecord := func() error {
		...
		if err := b.readFromUntil(rd, recordHeaderLen); err != nil {
			//fmt.Printf("  readClientHello err 1: %+v", err)
			return err
		}
		...
		//fmt.Printf("[INFO] vhost vers=%d\n", vers)
		n := int(b.data[3])<<8 | int(b.data[4])
		if n > maxCiphertext {
			//fmt.Printf("[ERROR] vhost n was %d and greater than maxCiphertext (%d). Version=%d (%v)\n", n, maxCiphertext, vers, b.data[0:5])
			return alertRecordOverflow
		}

		// First message, be extra suspicious:1
		...
		if (typ != recordTypeHandshake) || vers >= 0x1000 || n >= 0x3000 {
			// TODO: Playing Spotify tracks via Sonos throws a TLS error here. Version = 1024.
			fmt.Printf("  readClientHello unexpected message - ver: %d  n: %d\n ", vers, n)
			return alertUnexpectedMessage
		}

		if err := b.readFromUntil(rd, recordHeaderLen+n); err != nil {
			//fmt.Printf("  readClientHello err 2: %+v\n", err)
			return err
		}
		...
		if len(data) > maxPlaintext {
			fmt.Printf("[ERROR] vhost len(data) was %d and greater than maxPlaintext (%d)\n", len(data), maxPlaintext)
			return alertRecordOverflow
		}
	...
	if err := readRecord(); err != nil {
		//fmt.Printf("  readClientHello err 3: %+v\n", err)
	...
	for hand.Len() < 4+n {
		if err := readRecord(); err != nil {
			//fmt.Printf("  readClientHello err 4: %+v", err)
			return nil, err
		}
	}
