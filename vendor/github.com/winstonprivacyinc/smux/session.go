...
func (s *Session) OpenStream() (*Stream, error) {
	...
	//fmt.Printf("[DEBUG] smux.Session.go OpenStream() - opened a new stream. s.streams: %+v\n", s.streams)
	s.streamLock.Unlock()
	return stream, nil
}
...
func (s *Session) AcceptStream() (*Stream, error) {
	//fmt.Printf("smux.session.go AcceptStream()...\n")
	var deadline <-chan time.Time
	if d, ok := s.deadline.Load().(time.Time); ok && !d.IsZero() {
		//fmt.Printf("smux.session.go AcceptStream(). Setting timer: %v\n", time.Until(d))
	...
	case stream := <-s.chAccepts:
		//fmt.Printf("smux.session.go AcceptStream() Accept\n")
		return stream, nil
	case <-deadline:
		//fmt.Printf("smux.session.go AcceptStream() Deadline\n")
		return nil, errTimeout
	case <-s.die:
		//fmt.Printf("smux.session.go AcceptStream() Die\n")
		return nil, errors.New(errBrokenPipe)
	}
}
