...
type (
	...
	UDPSession struct {
		...
		// RLS 7/6/2018 - Callback functions to meter bandwidth
...
func (s *UDPSession) Read(b []byte) (n int, err error) {
	for {
		...
		if len(s.bufptr) > 0 { // copy from buffer into b
			...
			if s.ReadMeter != nil && n > 0 {
				s.ReadMeter(uint64(n))
			}
		...
		if size := s.kcp.PeekSize(); size > 0 { // peek data size from kcp
			...
			if len(b) >= size { // direct write to b
				s.kcp.Recv(b)
				s.mu.Unlock()
				if s.ReadMeter != nil && size > 0 {
					s.ReadMeter(uint64(size))
				}
				return size, nil
			}
		...
		case err = <-s.chErrorEvent:
			...
			//fmt.Printf("[DEBUG] xtaci.sess.go.Read - %d\n", n)
			if s.ReadMeter != nil && n > 0 {
				s.ReadMeter(uint64(n))
			}
			return n, err
...
func (s *UDPSession) Write(b []byte) (n int, err error) {
	for {
		...
		if s.kcp.WaitSnd() < int(s.kcp.snd_wnd) {
			...
			if s.WriteMeter != nil && n > 0 {
				//fmt.Printf("[DEBUG] xtaci.sess.go.Write - %d\n", n)
				s.WriteMeter(uint64(n))
			}
			return n, nil
		}
...
func (s *UDPSession) Close() error {
	...
	//fmt.Printf("[DEBUG] UDPSession.Close() - already closed? %t\n", s.isClosed)
	updater.removeSession(s)
	//fmt.Printf("[DEBUG] UDPSession.Close() - Closed connection\n")
...
type (
	...
	Listener struct {
		...
		wd              atomic.Value
		isclosed	bool			   // RLS - Set to true if closed.
	}
...
// RLS - 7/6/2018 - Allow caller to change the block cipher - doesn't work!
func (l *Listener) UpdateBlock(block *BlockCrypt) {
	l.block = *block
}
...
//fmt.Printf("[DEBUG] monitor - dataValid: %t\n", dataValid)
			if dataValid {
				var conv uint32
				convValid := false
				if l.fecDecoder != nil {
...
func (l *Listener) AcceptKCP() (*UDPSession, error) {
	...
	select {
	case <-timeout:
		//fmt.Printf("[DEBUG] AcceptKCP - timeout\n")
		return nil, &errTimeout{}
	case c := <-l.chAccepts:
		//fmt.Printf("[DEBUG] AcceptKCP - Accept\n")
		return c, nil
	case <-l.die:
		//fmt.Printf("[DEBUG] AcceptKCP - die\n")
...
func ListenWithOptions(laddr string, block BlockCrypt, dataShards, parityShards int) (*Listener, error) {
	...
	if err != nil {
		fmt.Printf("[DEBUG] Sess.go - ListenUDP @ %+v  err=%+v\n", udpaddr, err)
	}
	if err != nil {
...
// RLS - Added local address to be used for testing purposes
func DialWithOptions(raddr string, laddr string, block BlockCrypt, dataShards, parityShards int) (*UDPSession, error) {
	...
	if laddr != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "net.ResolveUDPAddr - Couldn't resolve local address")
		}
	}

	//fmt.Println()
	//fmt.Printf("[DEBUG] kcp/sess.go: DialWithOptions() - Connecting from localaddr %+v -> %+v \n", localaddr, udpaddr)
	udpconn, err := net.DialUDP("udp", localaddr, udpaddr)
	//fmt.Printf("[DEBUG] kcp/sess.go: DialWithOptions() - Completed UDP dial\n", localaddr, udpaddr)

	if err != nil {
		//if err != nil {
		//	fmt.Printf("[ERROR] DialWithOptions - localaddr: %+v  err %+v\n", localaddr, err)
		//}
		// TEST: Is specifying the local address causing the bind errors?
		if strings.Contains(err.Error(), "address already in use") {
			//fmt.Println("[DEBUG] DialWithOptions - Trying again without local address\n")
			udpconn, err = net.DialUDP("udp", nil, udpaddr)
		}
		//if err != nil {
		//	fmt.Printf("[ERROR] DialWithOptions (2nd try) - localaddr: %+v  err %+v\n", localaddr, err)
		//}
	}
	if err != nil {
